    ...
    case 'get_contacts':
        ...
        try {
            if ($current_user_role === 'teacher') {
                $sql_teacher = "SELECT school_id, std FROM teacher WHERE id = :id";                $stmt_teacher = $conn->prepare($sql_teacher);
                ...
                if ($teacher) {
                    ...
                    $sql_students = "
                        SELECT 
                            ...
                            s.std,
                            (SELECT COUNT(*) FROM messages WHERE sender_id = s.id AND receiver_id = :current_user_id AND is_read = FALSE) AS unread_count, /* <-- ADD THIS LINE */
                ...
                if ($student) {
                    ...
                    SELECT 
                        ...
                        (SELECT COUNT(*) FROM messages WHERE sender_id = t.id AND receiver_id = :current_user_id AND is_read = FALSE) AS unread_count,
                        (SELECT MAX(m.timestamp) FROM messages m WHERE (m.sender_id = t.id AND m.receiver_id = :current_user_id) OR (m.sender_id = :current_user_id AND m.receiver_id = t.id)) AS last_message_timestamp
                    FROM teacher t
                    WHERE t.school_id = :school_id AND :student_standard = ANY(t.std)
                    ORDER BY last_message_timestamp DESC, t.teacher_name ASC
                ";
