...
if (isset($_POST['download_pdf'])) {
    ...
    $dompdf->render();
    $dompdf->stream("Attendance_Report.pdf", ["Attachment" => 1]);
    exit();
}
...
if ($school_id) {
    try {
        ...
        $daily_query = "SELECT COUNT(*) as total, SUM(CASE WHEN status = 'Present' THEN 1 WHEN status = 'Half Day' THEN 0.5 ELSE 0 END) as present_total FROM attendance WHERE school_id = ? AND attendance_date = ?";
        $daily_stmt = $conn->prepare($daily_query);
        $daily_stmt->execute([$school_id, $selected_date]);
        ...
        $monthly_query = "SELECT COUNT(*) as total, SUM(CASE WHEN status = 'Present' THEN 1 WHEN status = 'Half Day' THEN 0.5 ELSE 0 END) as present_total FROM attendance WHERE school_id = ? AND EXTRACT(YEAR FROM attendance_date) = ? AND EXTRACT(MONTH FROM attendance_date) = ?";
        $monthly_stmt = $conn->prepare($monthly_query);
        $monthly_stmt->execute([$school_id, $year_month, $month_val]);
        ...
        $low_att_stmt->execute([$school_id, $low_attendance_threshold]);
        ...
        foreach ($roles_to_query as $current_role) {
            ...
            $staff_list_query = "SELECT id, {$map['name_col']} as name FROM {$current_role} WHERE school_id = ?";
            $staff_list_stmt = $conn->prepare($staff_list_query);
            $staff_list_stmt->execute([$school_id]);
            ...
            if (!empty($staff_ids)) {
                ...
                try {
                    ...
                    $params = array_merge($staff_ids, [$year_month, $month_val]);
...
<body id="page-top">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Attendance Analysis Report</h1>
    ...
    document.getElementById('download-full-report-btn').addEventListener('click', function() {
        ...
        const staffTableHtml = document.getElementById('staff-attendance-table').innerHTML;
