...
// Handle updating HR transport
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_transport'])) {
    $hr_transport_data = $_POST['hr_transport'] ?? [];
    $current_section = $_POST['current_section'] ?? 'school';
    $redirect_section = $current_section;
    $submitted_hr_ids = array_keys($hr_transport_data);
    $hr_to_update = [];

    if (!empty($submitted_hr_ids)) {
        $in_placeholders = implode(',', array_fill(0, count($submitted_hr_ids), '?'));
        $stmt_original = $conn->prepare("SELECT id, transport_mode, stop_id, self_transport_mode, vehicle_number, license_number FROM hr WHERE id IN ($in_placeholders) AND school_id = ?");
        $stmt_original->execute([...$submitted_hr_ids, $school_id]);
        $original_data = $stmt_original->fetchAll(PDO::FETCH_UNIQUE | PDO::FETCH_ASSOC);

        foreach ($hr_transport_data as $hr_id => $submitted_data) {
            $original = $original_data[$hr_id] ?? null;
    ...
    $actually_updated_count = count($hr_to_update);

    if ($actually_updated_count > 0) {
        try {
            $conn->beginTransaction();
            $stmt_update = $conn->prepare("UPDATE hr SET transport_mode = ?, stop_id = ?, self_transport_mode = ?, vehicle_number = ?, license_number = ? WHERE id = ? AND school_id = ?");
            
            foreach ($hr_to_update as $hr_id => $update_data) {
                $stmt_update->execute([$update_data['transport_mode'], $update_data['stop_id'], $update_data['self_transport_mode'], $update_data['vehicle_number'], $update_data['license_number'], (int)$hr_id, $school_id]);
            }
            
            $conn->commit();
            $plural = $actually_updated_count > 1 ? 's' : '';
            $success = "{$actually_updated_count} HR employee{$plural}'s transport information updated successfully!";
            log_interaction($role, $userId, "TRANSPORT: Updated transport details for {$actually_updated_count} HR employee(s).", $userName);

        } catch (PDOException $e) {
            $conn->rollBack();
            $errors[] = "Database update failed: " . $e->getMessage();
            log_interaction($role, $userId, "TRANSPORT ERROR: Failed to update HR employee transport. " . $e->getMessage(), $userName);
        }
    } else {
        $success = "No changes were detected.";
    }

    $redirect_url = "hr_transport.php?section=" . urlencode($redirect_section);
    if (!empty($success)) {
...
$hr_query = $conn->prepare("SELECT id, hr_name, phone, transport_mode, self_transport_mode, vehicle_number, license_number, stop_id FROM hr WHERE school_id = ? ORDER BY hr_name");
$hr_query->execute([$school_id]);
$hr_staff = $hr_query->fetchAll(PDO::FETCH_ASSOC);
...
$school_transport_hr = array_filter($hr_staff, function($h) {
    return $h['transport_mode'] === 'School Transport';
});
$self_transport_hr = array_filter($hr_staff, function($h) {
    return $h['transport_mode'] === 'Self Transport' || $h['transport_mode'] === 'Self';
});
...
<head>
    <meta charset="utf-8">
    <title>Manage HR Transport - School Management System</title>
...
?>                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Manage HR Transport</h1>
                        ...
                        <div class="card-body">
                            ...
                            <form method="POST">
                                ...
                                <div id="school-section" style="display: none;">
                                    <h5 class="my-3">School Transport HR Staff</h5>
                                    <div class="table-responsive">
                                        <table class="table table-bordered" width="100%" cellspacing="0">
                                            ...
                                            <tbody>
                                                <?php if (!empty($school_transport_hr)): ?>
                                                    <?php foreach ($school_transport_hr as $hr): ?>
                                                        <tr>
                                                            <td><?php echo htmlspecialchars($hr['hr_name']); ?></td>
                                                            <td><?php echo htmlspecialchars($hr['phone']); ?></td>
                                                            <td>
                                                                <select class="form-control form-control-sm transport-mode-select" name="hr_transport[<?php echo $hr['id']; ?>][transport_mode]" onchange="toggleInputs(this)">
                                                            ...
                                                            <td>
                                                                <div class="school-details">
                                                                    <select class="form-control form-control-sm" name="hr_transport[<?php echo $hr['id']; ?>][stop_id]">
                                                                        ...
                                                                        <?php
                                                                            ...
                                                                            foreach($all_stops as $stop) {
                                                                                ...
                                                                                $selected = ($hr['stop_id'] == $stop['id']) ? 'selected' : '';
                                                                ...
                                                                <div class="self-details" style="display:none;">
                                                                    <select class="form-control form-control-sm mb-2" name="hr_transport[<?php echo $hr['id']; ?>][self_transport_mode]" onchange="toggleSelfDetails(this)">
                                                                    ...
                                                                    <div class="self-details-fields" style="display:none;">
                                                                        <input type="text" class="form-control form-control-sm mb-2" name="hr_transport[<?php echo $hr['id']; ?>][vehicle_number]" placeholder="Vehicle Number">
                                                                        <input type="text" class="form-control form-control-sm" name="hr_transport[<?php echo $hr['id']; ?>][license_number]" placeholder="License Number">
                                                                    </div>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                ...
                                                <?php else: ?>
                                                    <tr>
                                                        <td colspan="4" class="text-center">No HR staff found with School Transport.</td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        </table>
                                ...
                                <div id="self-section" style="display: none;">
                                    <h5 class="my-3">Self Transport HR Staff</h5>
                                    <div class="table-responsive">
                                        <table class="table table-bordered" width="100%" cellspacing="0">
                                            <thead>
                                                <tr>
                                            ...
                                            <tbody>
                                                <?php if (!empty($self_transport_hr)): ?>
                                                    <?php foreach ($self_transport_hr as $hr): ?>
                                                        <tr>
                                                            <td><?php echo htmlspecialchars($hr['hr_name']); ?></td>
                                                            <td><?php echo htmlspecialchars($hr['phone']); ?></td>
                                                            <td>
                                                                <select class="form-control form-control-sm" name="hr_transport[<?php echo $hr['id']; ?>][transport_mode]" onchange="toggleInputs(this)">
                                                            ...
                                                            <td>
                                                                ...
                                                                <div class="self-details">
                                                                    <select class="form-control form-control-sm mb-2" name="hr_transport[<?php echo $hr['id']; ?>][self_transport_mode]" onchange="toggleSelfDetails(this)">
                                                                        <option value="">-- Select Mode --</option>
                                                                        <?php foreach ($self_transport_modes as $mode): ?>
                                                                            <option value="<?php echo htmlspecialchars($mode); ?>" <?php echo ($hr['self_transport_mode'] === $mode) ? 'selected' : ''; ?>>
                                                                                <?php echo htmlspecialchars($mode); ?>
                                                                            </option>
                                                                        <?php endforeach; ?>
                                                                    </select>
                                                                    <div class="self-details-fields" style="<?php echo ($hr['self_transport_mode'] === 'Bike' || $hr['self_transport_mode'] === 'Car') ? 'display:block;' : 'display:none;'; ?>">
                                                                        <input type="text" class="form-control form-control-sm mb-2" name="hr_transport[<?php echo $hr['id']; ?>][vehicle_number]" placeholder="Vehicle Number" value="<?php echo htmlspecialchars($hr['vehicle_number'] ?? ''); ?>">
                                                                        <input type="text" class="form-control form-control-sm" name="hr_transport[<?php echo $hr['id']; ?>][license_number]" placeholder="License Number" value="<?php echo htmlspecialchars($hr['license_number'] ?? ''); ?>">
                                                                    </div>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                ...
                                                <?php else: ?>
                                                    <tr>
                                                        <td colspan="4" class="text-center">No HR staff found with Self Transport.</td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        </table>
