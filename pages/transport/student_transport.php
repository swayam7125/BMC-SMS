...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_transport'])) {
    ...
    try {
        ...
        $stmt_fetch_original = $conn->prepare("SELECT transport_mode FROM student WHERE id = ?");
        ...
        foreach ($student_transport_data as $student_id => $data) {
            ...
            $stmt_fetch_original->execute([(int)$student_id]);
            ...
            $stmt_update->execute([$transport_mode, $stop_id, $self_transport_mode, $vehicle_number, $license_number, (int)$student_id, $school_id]);
        }
        $conn->commit();
                                                ...
                                                <?php if (!empty($school_transport_students)): ?>
                                                    <?php foreach ($school_transport_students as $student): ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <select class="form-control form-control-sm transport-mode-select" name="student_transport[<?php echo $student['id']; ?>][transport_mode]" onchange="toggleInputs(this)">
