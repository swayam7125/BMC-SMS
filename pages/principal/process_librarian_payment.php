...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $librarian_id = filter_input(INPUT_POST, 'librarian_id', FILTER_VALIDATE_INT);
    ...
    $present_days = filter_input(INPUT_POST, 'present_days', FILTER_VALIDATE_FLOAT);
    ...
    try {
        ...
        $check_stmt = $conn->prepare("SELECT id FROM librarian_payroll_records WHERE librarian_id = ? AND salary_month = ? AND salary_year = ?");
        $check_stmt->execute([$librarian_id, $salary_month, $salary_year]);
        if ($check_stmt->fetch()) {
            throw new Exception("Salary for this librarian for this month has already been processed.");
        }

        // 2. Insert the payment record
        $sql = "INSERT INTO librarian_payroll_records (librarian_id, principal_id, school_id, salary_month, salary_year, base_salary, total_working_days, present_days, absent_days, deduction_amount, net_salary_paid) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            $librarian_id, $principal_id, $school_id, $salary_month, $salary_year,
            $base_salary, $total_working_days, $present_days, $absent_days,
            $deduction_amount, $net_salary_paid
        ]);

        // 3. Create a notification for the librarian
        ...
        $log_message = "PAYROLL: Librarian ID {$librarian_id} salary processed for {$monthName} {$salary_year}. Net amount paid: {$currency_paid}.";
        log_interaction($role, $userId, $log_message, $acting_user_name);
        
        $success_message = "Salary has been processed successfully!";
        $redirect_url = "generate_librarian_payroll.php?month=" . urlencode($salary_month) . "&year=" . urlencode($salary_year) . "&success=" . urlencode($success_message);
        ...
        $redirect_url = "generate_librarian_payroll.php?month=" . urlencode($salary_month) . "&year=" . urlencode($salary_year) . "&error=" . urlencode($error_message);
        header("Location: " . $redirect_url);
        exit();
    }
} else {
    header("Location: generate_librarian_payroll.php");
    exit();
}
