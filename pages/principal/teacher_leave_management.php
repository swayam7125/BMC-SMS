...
try {
    ...
    $stmt_mark_all_read = $conn->prepare("UPDATE notifications SET is_read = TRUE WHERE user_id = ? AND type = 'leave_request' AND is_read = FALSE");
    $stmt_mark_all_read->execute([$current_user_id]);

    // Fetch pending leave applications
    $query_pending = "SELECT l.id, t.teacher_name, l.from_date, l.to_date, l.leave_type, l.reason, l.applied_on
    ...
    $query_history = "SELECT t.teacher_name, l.from_date, l.to_date, l.leave_type, l.reason, l.status, l.rejection_reason
                      FROM leave_applications l
                      JOIN teacher t ON l.teacher_id = t.id
                      WHERE l.status IN ('Approved', 'Rejected')
                      ORDER BY l.applied_on DESC
                      LIMIT 5"; // This is the new line added
    $stmt_history = $conn->prepare($query_history);
    $stmt_history->execute();
    $leave_history = $stmt_history->fetchAll(PDO::FETCH_ASSOC);

} catch (PDOException $e) {
    error_log("Principal Leave Requests Error: " . $e->getMessage());
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Teacher Leave Management</h1>
                                    ...
                                    <tbody>
                                        <?php if (!empty($leave_requests)): foreach ($leave_requests as $row): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="update_leave_status.php?id=<?php echo $row['id']; ?>&action=approve" class="btn btn-success btn-sm">Approve</a>
