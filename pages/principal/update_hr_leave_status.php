...
try {
    // Fetch HR ID and name for notification and logging
    $stmt_info = $conn->prepare("SELECT la.hr_id, h.hr_name FROM hr_leave_applications la JOIN hr h ON la.hr_id = h.id WHERE la.id = ?");
    $stmt_info->execute([$leave_id]);
    $leave_info = $stmt_info->fetch(PDO::FETCH_ASSOC);
    if (!$leave_info) {
        throw new Exception("Leave application not found.");
    }
    $hr_id = $leave_info['hr_id'];
    $hr_name = $leave_info['hr_name'];

    // Update the leave status
    $stmt = $conn->prepare("UPDATE hr_leave_applications SET status = ?, rejection_reason = ? WHERE id = ?");
    $stmt->execute([$status, $rejection_reason, $leave_id]);

    // Notify the HR staff member
    ...
    // Log the action
    $log_message = "LEAVE MGMT (HR): {$status} leave application ID {$leave_id} for {$hr_name}.";
    ...
    // Log the error
    log_interaction($role, $userId, "LEAVE MGMT (HR) ERROR: Failed to update leave status for application ID {$leave_id}. Error: " . $e->getMessage(), $userName);
