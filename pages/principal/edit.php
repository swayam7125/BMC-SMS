    ...
    if (empty($errors)) {
        ...
        try {
            ...
            mysqli_stmt_bind_param(
                $stmt_principal, "sssssssssdisi", 
                $image_path_for_db, $principal_name, $email, $phone, 
                $principal_dob, $gender, $blood_group, $address, 
                $qualification, $salary, $school_id, $batch, $principal_id
            );
...
<body id="page-top">
    ...
    <script>
        document.getElementById('principal_image').addEventListener('change', function(event) { if (event.target.files[0]) { document.getElementById('imagePreview').src = URL.createObjectURL(event.target.files[0]); } });
