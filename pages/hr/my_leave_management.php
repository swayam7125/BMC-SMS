...
try {
    if ($hr_id) {
        ...
        if (isset($_GET['notif_id']) && is_numeric($_GET['notif_id'])) {
            ...
            $stmt_mark_read->execute([$_GET['notif_id'], $hr_id]);
        }

        $stmt_user = $conn->prepare("SELECT hr_name, email FROM hr WHERE id = ?");
        $stmt_user->execute([$hr_id]);
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($from_date) || empty($to_date) || empty($reason)) {
                ...
                $stmt_insert = $conn->prepare("INSERT INTO hr_leave_applications (hr_id, from_date, to_date, reason, leave_type) VALUES (?, ?, ?, ?, ?)");
                $stmt_insert->execute([$hr_id, $from_date, $to_date, $reason, $leave_type]);

                // Find the principal to send a notification
                $stmt_principal = $conn->prepare("SELECT p.id, p.email, p.principal_name FROM principal p JOIN hr h ON p.school_id = h.school_id WHERE h.id = ?");
                $stmt_principal->execute([$hr_id]);
                if ($principal_data = $stmt_principal->fetch(PDO::FETCH_ASSOC)) {
                    $notification_message = "New leave request from " . htmlspecialchars($hr_name);
                    $link = "pages/principal/hr_leave_management.php";
                    $stmt_notify = $conn->prepare("INSERT INTO notifications (user_id, message, link, type) VALUES (?, ?, ?, 'hr_leave_request')");
        ...
        // Fetch Leave History
        $stmt_history = $conn->prepare("SELECT from_date, to_date, leave_type, reason, applied_on, status, rejection_reason FROM hr_leave_applications WHERE hr_id = ? ORDER BY applied_on DESC");
        $stmt_history->execute([$hr_id]);
                        ...
                        <div class="card-body">
                            <form method="POST" action="my_leave_management.php">
                                <div class="form-group">
                                    <label>Name</label>
                                    <input type="text" class="form-control" value="<?php echo htmlspecialchars($hr_name); ?>" readonly>
                                </div>
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
