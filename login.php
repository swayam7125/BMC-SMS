...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // --- CHANGE 1: SELECT THE NEW 'account_status' COLUMN ---
        ...
        if ($stmt) {
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    // --- CHANGE 2: CHECK IF THE ACCOUNT IS SUSPENDED ---
                    if ($user['account_status'] === 'suspended') {
                        // If suspended, set an error message and stop the login process.
                                ...
                                <div class="p-5">
                                    ...
                                    <form class="user" method="POST">
                                        <div class="form-group">
                                            <input type="email" class="form-control form-control-user opacity-100" name="email" placeholder="Enter Email Address..." required>
                                        </div>
                                        <div class="form-group">
                                            <input type="password" class="form-control form-control-user opacity-100" name="password" placeholder="Password" required>
                                        </div>
