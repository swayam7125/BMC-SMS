...
class DataTableManager {
  ...
  initTeacherTable(config) {
    if (
      $("#teacherListTable").length > 0 &&
      $.fn.DataTable &&
      !$.fn.DataTable.isDataTable("#teacherListTable")
    ) {
  ...
  initStudentTable(config) {
    if (
      $("#studentListTable").length > 0 &&
      $.fn.DataTable &&
      !$.fn.DataTable.isDataTable("#studentListTable")
    ) {
...
class FormValidator {
  ...
  validateField($field) {
    ...
    if (
      ...
      required &&
        !fieldValue &&
        ((isValid = !1), (errorMessage = "This field is required")),
      "email" === fieldType &&
        fieldValue &&
        !this.isValidEmail(fieldValue) &&
        ((isValid = !1), (errorMessage = "Please enter a valid email address")),
      $field.hasClass("phone") &&
        fieldValue &&
        !this.isValidPhone(fieldValue) &&
        ((isValid = !1), (errorMessage = "Please enter a valid phone number")),
      "password" === fieldType &&
        fieldValue &&
        fieldValue.length < 6 &&
        ((isValid = !1),
        (errorMessage = "Password must be at least 6 characters long")),
      $field.hasClass("confirm-password"))
    ) {
      const originalPassword = $(
        `input[name="${fieldName.replace("_confirm", "")}"]`
      ).val();
      fieldValue !== originalPassword &&
        ((isValid = !1), (errorMessage = "Passwords do not match"));
    }
    return (
      isValid ||
...
class NotificationManager {
  ...
  updateBadges(counts) {
    for (const [type, count] of Object.entries(counts)) {
      const $links = $(
        `.nav-link[data-notification-type="${type}"], .collapse-item[data-notification-type="${type}"]`
      );
      $links.each(function () {
        const $link = $(this);
  ...
  showToast(message, type = "info") {
    ...
    const toast = $(
      '<div id="' +
        toastId +
        '" class="toast" role="alert" aria-live="assertive" aria-atomic="true" style="position: fixed; top: 20px; right: 20px; z-index: 1050;">' +
  ...
  markAsRead(type) {
    $.post(
      "/BMC-SMS/includes/ajax/mark_notifications_read.php",
      {
        type: type,
      },
      (response) => {
        if (response.success) {
          $(
            `.nav-link[data-notification-type="${type}"] .badge-counter, .collapse-item[data-notification-type="${type}"] .badge-counter`
          ).fadeOut();
        }
      },
      "json"
